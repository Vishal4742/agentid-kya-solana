...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (typeof rawName !== "string" || rawName.length < 3 || rawName.length > 68) {
        return res.status(400).json({ error: "Agent name must be 3–64 characters" });
    }
    ...
    try {
        ...
        if (!match) {
            ...
            const placeholder = {
                ...
                properties: { category: "identity", non_transferable: true, soul_bound: true },
            };
        ...
        const serviceCategory = CATEGORIES[acc.serviceCategory] ?? "Information Technology Services";
        const registeredDate = new Date(acc.registeredAt.toNumber() * 1000).toISOString().split("T")[0];
        ...
        const metadata = {
            ...
            attributes: [
                ...
                { trait_type: "Max USDC per Tx", value: `$${(acc.maxTxSizeUsdc.toNumber() / 1_000_000).toFixed(0)}` },
                { trait_type: "Total Transactions", value: acc.totalTransactions.toNumber() },
            ],
            properties: {
                category: "identity",
