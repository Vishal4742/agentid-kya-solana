...
const agentid_program_json_1 = __importDefault(require("../../target/idl/agentid_program.json"));
...
const oracleWallet = new anchor_1.Wallet(oracleKeypair);
console.log(`🔑 Oracle Authority: ${oracleKeypair.publicKey.toBase58()}`);
// Setup Anchor Program
const connection = new web3_js_1.Connection(RPC_URL, "confirmed");
const provider = new anchor_1.AnchorProvider(connection, oracleWallet, {
    preflightCommitment: "confirmed",
});
// Program ID is read from the IDL (Anchor 0.30)
const program = new anchor_1.Program(agentid_program_json_1.default, provider);
const PROGRAM_ID = program.programId;
console.log(`📜 Loaded Program: ${PROGRAM_ID.toBase58()}`);
// Seed prefixes from Anchor program
const CONFIG_SEED = Buffer.from("config");
const [configPda] = web3_js_1.PublicKey.findProgramAddressSync([CONFIG_SEED], PROGRAM_ID);
// ── Webhook Endpoint ────────────────────────────────────────────────────────
app.post("/webhook", async (req, res) => {
    ...
    for (const tx of transactions) {
        ...
        const isOurs = logs.some((l) => l.includes(PROGRAM_ID.toBase58()));
        if (!isOurs)
            continue;
        ...
        try {
            ...
            const accountKeys = tx.accountData?.map((a) => a.account) ||
                ...
                [];
            let agentIdentityPda = null;
            let agentData = null;
            for (const key of accountKeys) {
                try {
                    const pk = new web3_js_1.PublicKey(key);
                    if (pk.equals(PROGRAM_ID) || pk.equals(oracleKeypair.publicKey))
                        continue;
                    const data = await program.account.agentIdentity.fetch(pk);
                    agentIdentityPda = pk;
                    agentData = data;
                    break;
                }
            ...
            if (verLevel === 1)
                scoreVerification = 50; // EmailVerified
            if (verLevel === 2)
                scoreVerification = 100; // KYBVerified
            if (verLevel === 3)
                scoreVerification = 200; // Audited
...
node_cron_1.default.schedule("0 * * * *", async () => {
    ...
    try {
        const allAgents = await program.account.agentIdentity.all();
        ...
        for (const agent of allAgents) {
            ...
            let scoreVerification = 0;
            if (verLevel === 1)
                scoreVerification = 50;
            if (verLevel === 2)
                scoreVerification = 100;
            if (verLevel === 3)
                scoreVerification = 200;
            const totalScore = Math.floor(scoreSuccess + scoreRating + scoreLongevity + scoreVolume + scoreVerification);
            const newScore = Math.min(Math.max(totalScore, 0), 1000);
